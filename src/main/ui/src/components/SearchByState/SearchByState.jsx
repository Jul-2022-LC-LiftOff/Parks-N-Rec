...
export default function SearchByState() {
  ...
  React.useEffect(() => {
    const url = `https://developer.nps.gov/api/v1/parks?parkCode="all"&limit=469&api_key=JMZizGv6gAcjBzoD4TbqW9RQSe9K8fEt9Cdb2Zta`;
    fetch(url)
  ...
  if (data.length > 0) {
    data.filter((parkApi) => parkApi.states.match(data));
  }
  ...
  return (
    <div>
      <div>
        ...
        {data
          .filter((park) => filterParkStates(park))
          .map((park, key) => (
                         <div key={key}>
                         {park.states}
                         <br/>
